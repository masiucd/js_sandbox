...
import {
  LordActionTypes, CatchLordErrors, GetCharAction, GetMoviesAction, GetQuotesAction,
} from './ring.types';
...
export const getMovies = () => async (dispatch: Dispatch<GetMoviesAction|CatchLordErrors>) => {
  try {
    const config = {
      headers: {
        // 'Content-Type': 'application/json',
    ...
    const res = await fetch('https://the-one-api.herokuapp.com/v1/movie', config);
    const resBody = await res.json();
    dispatch({
...
export const getQuotes = () => async (dispatch: Dispatch<GetQuotesAction|CatchLordErrors>) => {
  try {
    const config = {
      headers: {
        // 'Content-Type': 'application/json',
    ...
    const res = await fetch('https://the-one-api.herokuapp.com/v1/quote', config);
    const resBody = await res.json();
    dispatch({
